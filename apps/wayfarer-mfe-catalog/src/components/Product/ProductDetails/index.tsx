  ...
  return (
    <Box maxW="container.md" mx="auto" p={5}>
      <HStack gap={8} align="start">
        {/* Product Image */}
        <Image src={product.image} alt={product.title} boxSize="300px" objectFit="contain" borderRadius="md" />

        {/* Product Details */}
        <VStack align="start" gap={4}>
          <Text fontSize="2xl" fontWeight="bold">{product.title}</Text>
          <Badge colorScheme="blue" fontSize="lg">{product.category}</Badge>
          <Text fontSize="xl" color="green.500" fontWeight="bold">${product.price}</Text>
          <Text>{product.description}</Text>
          <HStack gap={4}>
            <Button colorScheme="blue" size="lg">Buy Now</Button>
            <Button colorScheme="gray" variant="outline">Add to Wishlist</Button>
          </HStack>
          ...
          <Text fontSize="xl" color="teal.500" fontWeight="bold">Specifications</Text>
          <Text>{product.description}</Text>
        </VStack>
      </HStack>
    </Box>
